...
export class ShellComponent implements OnInit {
  ...
  profissionalNav: NavItem[] = [
    { label: 'Meu Cadastro',  icon: 'user',      route: '/profissional/meu-cadastro' },
  ...
  ngOnInit() {
    this.theme = (localStorage.getItem('clinica_theme') as 'dark' | 'light') || 'light';
    this.applyTheme();
    this.router.events
  ...
  get currentPageLabel(): string {
    return this.navItems.find(item => this.isActive(item.route))?.label || 'Painel';
  }
  get sectionLabel(): string {
    return this.auth.isAdmin ? 'Painel Administrativo' : 'Operacao Clinica';
  }
